...
resource "aws_launch_configuration" "ec2" {
    ...
    user_data                   = var.cluster_resources[count.index]["user_data"]
    spot_price                  = lookup(var.cluster_resources[count.index], "spot_price", null)

    root_block_device {
            volume_type             = lookup(var.cluster_resources[count.index], "volume_type", null)
            volume_size             = lookup(var.cluster_resources[count.index], "volume_size", null)
            delete_on_termination   = lookup(var.cluster_resources[count.index], "delete_on_termination", null)   
    }
...
resource "aws_autoscaling_group" "ec2" {
    ...
    min_size                    = lookup(var.cluster_resources[count.index], "min_size", null)
    max_size                    = lookup(var.cluster_resources[count.index], "max_size", null)
    desired_capacity            = lookup(var.cluster_resources[count.index], "desired_capacity", null)
    health_check_type           = lookup(var.cluster_resources[count.index], "health_check_type", null)
    health_check_grace_period   = lookup(var.cluster_resources[count.index], "health_check_grace_period", null)
    default_cooldown            = lookup(var.cluster_resources[count.index], "default_cooldown", null)
